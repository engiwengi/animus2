...
//         task::{accept::AcceptConnectionsTask, recv::ReceivePacketsTask,
// send::SendPacketsTask},     },
// };
//
...
//     ) -> Self
//     where
//         A: AsyncAccept<Connection = quinn::Connection> + Unpin + Send +
// 'static,     {
//         let (stop, quit_rx) = watch::channel(());
//         let (new_connections_tx, new_connections) =
...
//     }
//
//     pub fn spawn_tasks_for_new_connections<'a, 'd, F>(&mut self, mut
// on_new_connection: F)     where
//         F: FnMut(mpsc::UnboundedSender<EncodedPacket>, NetworkId),
...
//
//             let mediator =
// AnyPacketMediator::new(Arc::clone(&self.packet_senders));             let
// receive_task = broadcast_disconnect.clone();             let stop =
// self.stop.subscribe();             let conn = connection.value.clone();
//             tokio::task::spawn(async move {
